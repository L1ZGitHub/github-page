...
**Supply chain attacks.** The open-source AI stack has many dependencies: model weights, tokenizers, agent frameworks, tool plugins, vector databases, [embedding models](/blog/embedding-models-compared-openai-vs-open-source). A compromised component anywhere in this chain, a backdoored fine-tune, a malicious plugin, a tampered embedding model, can compromise the entire system. The software supply chain security problem, already severe for traditional software, is amplified when the "software" includes neural network weights that are opaque to inspection.
