...
- **Agents aggregate across sources.** A multi-tool agent, as described in my earlier posts on Agentic RAG and [Building a Multi-Agent AI System](/blog/building-a-multi-agent-ai-system), naturally combines information from multiple retrieval backends. That combination can itself constitute MNPI even when individual sources do not.
...
Every retrieval call must be filtered by the requesting user's clearance and deal scope. This is the same principle I described in [Building Production-Ready RAG Systems](/blog/building-production-ready-rag-systems), but with financial-specific access controls.
...
This connects to the ideas in [Introduction to Differential Privacy for NLP](/blog/introduction-to-differential-privacy-for-nlp), where we discussed that privacy controls are most effective when applied as early as possible in the data pipeline.
...
This is analogous to the evaluation techniques in [Evaluating RAG System Performance](/blog/evaluating-rag-system-performance), but with a compliance lens rather than a pure accuracy lens.
