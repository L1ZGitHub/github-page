...
- **Agents aggregate across sources.** A [multi-agent system](/blog/building-a-multi-agent-ai-system) with access to multiple retrieval backends naturally combines information from several sources. That combination can itself constitute MNPI even when individual sources do not.
...
Multiple data feeds are indexed into the same vector store or knowledge base. Deal A's private information sits in the same index as Deal B's alternative data. Unless retrieval is strictly partitioned, any query can reach any document. Choosing the right [vector database](/blog/understanding-vector-databases) and configuring proper namespace isolation is the first line of defense.
...
LLMs can memorize and leak information from their context. If sensitive data enters the context window, it can influence the output in ways that are [difficult to audit](/blog/data-privacy-in-the-age-of-large-language-models). This risk grows with context size and the number of sources feeding into a single prompt.
...
- Run adversarial prompt tests to see if the agent can be tricked into ignoring deal boundaries. The same [prompt injection techniques](/blog/ai-security-how-agents-like-openclaw-can-be-exploited) used against general-purpose agents apply here, with higher stakes.

[Systematic evaluation](/blog/evaluating-rag-system-performance) of retrieval accuracy and filter effectiveness should be part of the regular compliance review cycle, not a one-time exercise.
