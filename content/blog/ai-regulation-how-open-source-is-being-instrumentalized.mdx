...
The problem is not with these arguments but with how the term "open source" is being stretched to cover practices that deliver none of these benefits.
...
- **GLM-5 from Zhipu AI**: Released under Apache 2.0, a genuinely open license with no usage restrictions. However, the model was trained on proprietary Chinese-language datasets that are not publicly available, and the Ascend-specific training infrastructure is not reproducible by most organizations. This pattern is [increasingly common among Chinese AI labs](/blog/chinese-ai-models-catching-up-and-surpassing-the-west): the weights are open, but the process that produced them is not.
...
- **OpenClaw**: An autonomous agent framework that releases its full codebase, but this openness created immediate [security consequences](/blog/ai-security-how-agents-like-openclaw-can-be-exploited) when the code was deployed in production without adequate hardening.
...
A related concern is evaluation governance. When we say a model is "safe" or "aligned," we are making a claim that depends on specific benchmarks, specific evaluation methodologies, and specific definitions of harm. These standards are currently set by the same organizations that train the models.

The open-source community has made progress here. Efforts like EleutherAI's Language Model Evaluation Harness, the Open LLM Leaderboard, and various community-driven red-teaming initiatives provide independent [evaluation infrastructure](/blog/evaluating-rag-system-performance). But these efforts are chronically underfunded relative to their importance, and they struggle to keep pace with the rapid model release cadence.
...
If European policymakers want meaningful AI transparency, several concrete steps would help.
