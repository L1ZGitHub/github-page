...
- **Retriever** - uses semantic search over internal docs (see [Semantic Search vs Keyword Search: When to Use What](/blog/semantic-search-vs-keyword-search-when-to-use-what))
...
- Knows which vector database to use (see Vector Database Performance Benchmarks and [Understanding Vector Databases: A Beginner's Guide](/blog/understanding-vector-databases))
- Knows which collections to query
- Applies filtering based on tenant, region, or privacy policies

This centralizes retrieval logic, which is crucial when you have compliance or privacy constraints such as those I described in [Data Privacy in the Age of Large Language Models](/blog/data-privacy-in-the-age-of-large-language-models).

### Confidential and public contexts

With strong privacy boundaries, I often split agents into:

- **Private context agent** - has access to sensitive user data, uses privacy-preserving NLP techniques similar to those in [Privacy-Preserving NLP: Protecting Sensitive Data in Language Models](/blog/privacy-preserving-nlp)
...
This is where articles like Embedding Models Compared: OpenAI vs Open-Source and [Fine-Tuning LLMs on Custom Data: A Practical Guide](/blog/fine-tuning-llms-custom-data) become relevant. The better your base models and embeddings, the more reliable each agent becomes.
...
- [FinAgent and Beyond: Multimodal Foundation Agents for Trading](/blog/finagent-and-beyond-multimodal-foundation-agents-for-trading)
