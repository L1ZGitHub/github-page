...
LangChain and LangGraph take a different path. They let you keep the high-level agent abstraction, while still exposing the graph underneath - the actual states, transitions, and tools used by the model. For production systems, especially those involving [Retrieval-Augmented Generation](/blog/retrieval-augmented-generation-a-complete-guide) (RAG) and privacy constraints, this explicit control is not a luxury. It is the difference between “it works on my laptop” and something you can trust.
...
If you have read my pieces on Retrieval-Augmented Generation or [Building Production-Ready RAG Systems](/blog/building-production-ready-rag-systems), you already know that the “tool” can be a retrieval call, a vector database query, or a custom microservice. LangChain wraps these into a unified interface.
...
You describe the workflow declaratively, then LangGraph executes it. If you are familiar with my article on [Chunking Strategies for RAG Pipelines](/blog/chunking-strategies-for-rag-pipelines), think of LangGraph as the orchestration layer that stitches all those components together.
...
Most interesting agents need context. That is where your RAG stack comes in: embeddings, vector databases, and retrieval logic, as covered in Retrieval-Augmented Generation and [Understanding Vector Databases](/blog/understanding-vector-databases).
...
4. **Use embeddings consistently**  - as in [Embedding Models Compared: OpenAI vs Open-Source](/blog/embedding-models-compared-openai-vs-open-source), decide early which embedding model to standardize on, and do not mix them within the same index.
