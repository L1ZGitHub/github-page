...
Here is a minimal example using regex for basic PII and secret patterns. In practice I combine it with ML-based [NER detectors](/blog/named-entity-recognition-with-modern-nlp) and [differential privacy techniques](/blog/introduction-to-differential-privacy-for-nlp) for stronger guarantees.
...
When you set up a [vector database](/blog/understanding-vector-databases) for RAG, adopt these patterns from the start:
...
When building tests or evaluation sets for [RAG system performance](/blog/evaluating-rag-system-performance):
...
This looks more complex than "call the LLM API from a Flask app", but the complexity is what buys you a meaningful reduction in privacy risk. Systems that combine [multimodal inputs](/blog/multimodal-ai-combining-vision-and-language-models) with text only increase the attack surface, so the layered approach matters even more.
