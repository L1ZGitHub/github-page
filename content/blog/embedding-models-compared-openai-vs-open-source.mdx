...
Most production [RAG](/blog/retrieval-augmented-generation-a-complete-guide) systems quietly depend on a single workhorse: the embedding model. When retrieval feels “magic” or inexplicably bad, it almost always comes down to how we turn text into vectors.
...
- Domain-specific models (code, legal, biomedical, multilingual, and increasingly [multimodal](/blog/multimodal-ai-combining-vision-and-language-models))
...
You can choose 384, 768, 1024, 2048 dimensional models depending on your latency/accuracy tradeoff and how it interacts with your [vector database's](/blog/understanding-vector-databases) indexing and storage strategy.
...
Benchmarks like MTEB are helpful to narrow the search space, but they are not your ground truth. You should [evaluate embeddings on task-specific metrics](/blog/evaluating-rag-system-performance), the same way you would evaluate any other component of a retrieval pipeline.
