...
The term "compound AI systems" keeps surfacing in 2026 literature, and for good reason. A compound system is not a single model. It is a composition of models, retrievers (often backed by [vector databases](/blog/understanding-vector-databases)), tools, validators, and orchestration logic that together solve a task. Multi-agent architectures are the natural way to structure these compositions.
...
- **Quality signals**: validation pass rates, user feedback correlation with plan structure (see [evaluating RAG system performance](/blog/evaluating-rag-system-performance) for related metrics)
...
**Agent count scaling**: most production systems use 3 to 8 specialized agents. Beyond that, coordination overhead dominates. If you need more specialization, consider hierarchical orchestration where a top-level orchestrator delegates to sub-orchestrators, each managing a small team. Agents that handle images or video alongside text add another dimension to this coordination (see [multimodal AI](/blog/multimodal-ai-combining-vision-and-language-models)).
...
3. **Graceful degradation**: if a non-critical agent is unavailable, skip it and mark the output as partial. A [portfolio analysis](/blog/ai-and-finance-how-autonomous-agents-are-transforming-trading) without the sentiment agent is still useful if the quantitative agents succeeded. Always communicate the degradation to the user.
