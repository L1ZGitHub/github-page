...
If you work with [vector databases](/blog/understanding-vector-databases), you will have seen similar metrics in a pure search context. For RAG, I recommend optimizing recall first, then improving precision with reranking or better chunking.
...
Human evaluation is the gold standard, but it does not scale. For day-to-day iterations, an LLM judge is a useful approximation. Understanding [how evaluation frameworks go beyond simple perplexity](/blog/llm-evaluation-frameworks-beyond-perplexity) helps frame what these judges actually measure.
...
then do not touch the LLM yet. Fix retrieval: better embeddings, better chunking, maybe hybrid search. The same principles that apply to [combining vision and language in multimodal systems](/blog/multimodal-ai-combining-vision-and-language-models) apply here: evaluation metrics keep you from optimizing the wrong component.
