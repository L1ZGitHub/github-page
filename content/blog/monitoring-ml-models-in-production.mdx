...
1. **Data dependence** - Model performance depends on input data distributions, which change over time.
2. **Delayed ground truth** - Labels often arrive late, or never at all (e.g. RAG answers in a chatbot).
3. **Non-determinism** - With LLMs and RAG systems especially, the same input may produce different outputs.
...
If you already have basic HTTP metrics from a [FastAPI + Docker deployment](/blog/deploying-ml-models-with-fastapi-and-docker), extending them to ML-specific endpoints is straightforward.
...
In RAG systems, data quality also involves your [vector database](/blog/understanding-vector-databases): chunk size, indexing lag, and corrupted embeddings all impact relevance. Monitoring ingestion lag and document counts per collection catches these problems early.
...
The same join-and-compute pattern applies to [RAG evaluation](/blog/evaluating-rag-system-performance), though the metrics differ.
...
You can integrate these batch jobs into your [CI/CD pipeline](/blog/cicd-pipelines-for-machine-learning-projects) so every new model ships with a pre-defined monitoring config.
...
The same principle applies to [multimodal AI systems](/blog/multimodal-ai-combining-vision-and-language-models) where each model component (vision encoder, language model, fusion layer) should have separate metrics so you can reason about its contribution.
