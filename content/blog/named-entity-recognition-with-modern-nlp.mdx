...
- In [privacy-preserving workloads](/blog/privacy-preserving-nlp), NER is a workhorse for de-identification, for example masking names, emails, or IDs.
...
- For high throughput systems, try `en_core_web_md` or `en_core_web_sm` and benchmark NER throughput on your target hardware.
...
This approach works well for de-identification: before you even think about [differential privacy](/blog/introduction-to-differential-privacy-for-nlp) noise budgets, remove or mask obvious identifiers with deterministic rules.
...
Before indexing documents into a [vector database](/blog/understanding-vector-databases), NER can:
...
When [evaluating a RAG system](/blog/evaluating-rag-system-performance), it is often useful to measure entity-specific metrics:
...
- **Model size vs latency**
  Distilled or smaller models often offer a much better latency-accuracy tradeoff. Measure it on representative data before committing to a large model.
