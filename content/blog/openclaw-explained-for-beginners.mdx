...
The underlying architecture is straightforward. OpenClaw sits between a large language model (the "brain") and a set of **tool integrations** (the "hands"). The LLM decides what to do; the tools execute it. This is the same pattern described by the [Model Context Protocol](/blog/model-context-protocol-connecting-llms-to-external-tools), which standardizes how LLMs communicate with external tools.
