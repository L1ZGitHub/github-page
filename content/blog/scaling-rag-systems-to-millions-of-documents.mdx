...
Once you pass a few hundred thousand documents, naive vector storage will hurt you. If you are new to the space, [Understanding Vector Databases](/blog/understanding-vector-databases) covers the foundations; here we need to think about layout and sharding.
...
Your [chunking strategy](/blog/chunking-strategies-for-rag-pipelines) dictates both index size and retrieval precision, so get that right before worrying about infrastructure.
...
At millions of documents, naive retrieval often dominates end-to-end latency. If you want hard numbers on different engines, the [vector database benchmarks](/blog/vector-database-performance-benchmarks) are a useful reference.
...
Make sure recall stays acceptable while you tune for latency. [Evaluating RAG system performance](/blog/evaluating-rag-system-performance) covers practical metrics and methods for this.
