...
The MLP is often 4x larger than `d_model` and is a major contributor to compute. When optimizing for inference cost, decreasing `d_ff` or using [low-rank adapters like LoRA](/blog/fine-tuning-open-source-llms-with-lora-and-qlora) can be effective, manipulating these linear layers instead of full fine-tuning.
...
Most [embedding models](/blog/embedding-models-compared-openai-vs-open-source) are encoder-only transformers with pooling and normalization.
...
This is essentially what most open-source embedding models do. The resulting vectors are then stored and queried in [vector databases](/blog/understanding-vector-databases) optimized for nearest-neighbor search.
...
If you are already comfortable with transformers, the next step is experimenting with [fine-tuning on your own data](/blog/fine-tuning-llms-custom-data) and wiring these models into proper services.
