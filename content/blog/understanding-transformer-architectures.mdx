...
This is the primitive that everything else builds on. For RAG systems (see "[Retrieval-Augmented Generation: A Complete Guide](/blog/retrieval-augmented-generation-a-complete-guide)" and "Building Production-Ready RAG Systems"), you are indirectly tuning these attention patterns through prompt construction and retrieval.
...
If you have read "[Fine-Tuning LLMs on Custom Data: A Practical Guide](/blog/fine-tuning-llms-custom-data)", you have seen low-rank adaptation (LoRA) and similar tricks that manipulate these linear layers instead of full fine-tuning.
...
This connects directly to "Semantic Search vs Keyword Search" and "[Embedding Models Compared: OpenAI vs Open-Source](/blog/embedding-models-compared-openai-vs-open-source)". Most embedding models are encoder-only transformers with pooling and normalization.
...
This is essentially what most open-source embedding models do, as discussed in "Embedding Models Compared: OpenAI vs Open-Source" and "[Understanding Vector Databases: A Beginner's Guide](/blog/understanding-vector-databases)".

## Privacy and attention control

In "[Privacy-Preserving NLP: Protecting Sensitive Data in Language Models](/blog/privacy-preserving-nlp)" I discussed strategies to avoid leaking sensitive data. Transformers give you several hooks for this.
